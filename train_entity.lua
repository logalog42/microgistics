...
DEBUG_MODE = minetest.settings:get_bool("microgistics_debug", false)
...
local function spawn_passengers(train, pos)
	...
	if minetest.settings:get_bool("microgistics_particles", true) then
...
local function should_collect_train(player)
	...
	local has_train = inv:contains_item("main", "microgistics:train")
...
local function train_drive(train, dtime)
	...
	local new_pos, new_dir, stop = Microgistics.get_next_pos(train, dtime)
	train.object:set_yaw(Microgistics.dir_to_yaw(new_dir))
...
minetest.register_entity("microgistics:train", train_entity)


minetest.register_craftitem("microgistics:train", {
	description = "Train: " ..
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if Microgistics.is_rail(under) then
			minetest.add_entity(pointed_thing.under, "microgistics:train")
		elseif Microgistics.is_rail(pointed_thing.above) then
			minetest.add_entity(pointed_thing.above, "microgistics:train")
		else
			return
		end
